...
def recognize_image(image):
    ...
    frame = cvimg                           #Grab a single frame of video
...
def attendance_image(image, batch, section):
    date="Date:" + str(datetime.now().strftime('%d-%m-%Y'))                                  #Load current date
    time="Time:" + str(datetime.now().strftime('%H;%M;%S'))                                  #Load current time
    dtt= str(datetime.now().strftime('%d-%m-%Y  %H;%M;%S'))                                  #Load current date and time
    filename= dtt + " .avi"                                                                  #make filename for stored video

    pathss=r'/home/hasan/Desktop/moiz_proj/Surveillance/'+batch+'/'+section                             #path for the dataset for respective batches,sections
    ...
    os.chdir(r"/home/hasan/Desktop/moiz_proj/Surveillance/video")           #change directory to loaction where video will be stored
    # video_capture = cv2.VideoCapture(0)                                     #initialize video capturing
    fourcc=cv2.VideoWriter_fourcc(*'XVID')                                  #sets the video codecs that is about to be saved
    out=cv2.VideoWriter(filename,fourcc,24.0,(640,480))                     #sets the filename,type,fps,resolution of the video
    ...
    os.chdir(r"/home/hasan/Desktop/moiz_proj/Surveillance/Attendance Record")#change the directory to the excel file of attendace storage loaction
    ...
    retval, buffer = cv2.imencode('.jpg', frame)
    jpg_as_text = base64.b64encode(buffer)
    return jpg_as_text                                                        #close worksheet
    # Release webcam
    # video_capture.release()
    # cv2.destroyAllWindows()
